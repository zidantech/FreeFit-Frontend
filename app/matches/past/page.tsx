...
export default function PastMatchesPage() {
  ...
  const fetchPastMatches = async () => {
    try {
      setLoading(true);
      const res = await matchesAPI.getPastMatches();
      const items = res?.data || res?.past_matches || (Array.isArray(res) ? res : []);
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-cyan-500/20 pb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Completed match results and scores</p>
              </div>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-16 bg-[#0f1535]/60 rounded-xl border border-cyan-500/20 max-w-xl mx-auto">
              <History className="w-14 h-14 mx-auto mb-4 text-cyan-400 opacity-60" />
              <h3 className="text-lg font-semibold text-white mb-1">No Past Matches Found</h3>
              <p className="text-gray-400 text-sm">Completed matches will appear here.</p>
            </div>
          )}
        </div>
      </div>
